...
var _default = new(_plugin().Packager)({
	async loadConfig({
		...
		const pkg = await config.getConfigFrom(_path().default.join(options.projectRoot, "index"), ["package.json"])
		const name = (_pkg$contents$name = pkg === null || pkg === void 0 ? void 0 : (_pkg$contents = pkg.contents) === null || _pkg$contents === void 0 ? void 0 : _pkg$contents.name) !== null && _pkg$contents$name !== void 0 ? _pkg$contents$name : ""
		return {
	...
	async package({
		...
		if(contents == null){
			/** <PATCH> **/
			// const packager = bundle.env.shouldScopeHoist ? new _ScopeHoistingPackager.ScopeHoistingPackager(options, bundleGraph, bundle, (0, _nullthrows().default)(config).parcelRequireName) : new _DevPackager.DevPackager(options, bundleGraph, bundle, (0, _nullthrows().default)(config).parcelRequireName);
			// if(!bundle.env.shouldScopeHoist){
			// 	throw new Error("Dev packager is not supported")
			// }
			const packager = new _ScopeHoistingPackager.ScopeHoistingPackager(options, bundleGraph, bundle, (0, _nullthrows().default)(config).parcelRequireName);
			/** </PATCH> **/
			({
				contents,
				map
