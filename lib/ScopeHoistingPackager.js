...
class ScopeHoistingPackager {
	...
	constructor(options, bundleGraph, bundle, parcelRequireName) {
		...
		const OutputFormat = OUTPUT_FORMATS[this.bundle.env.outputFormat]
		...
		/** <PATCH> **/
		this.patch = new Patch(bundle, bundleGraph)
		/** </PATCH> **/
	}
	...
	async package() {
		...
		if(this.bundle.env.isLibrary || this.bundle.env.outputFormat === "commonjs"){
			const bundles = this.bundleGraph.getReferencedBundles(this.bundle)
		...
		const processAsset = asset => {
			const [content, map, lines] = this.visitAsset(asset)
		...
		const [prelude, preludeLines] = this.buildBundlePrelude()
		res = prelude + res
		...
		for(const entry of entries){
			if(this.wrappedAssets.has(entry.id) && !this.isScriptEntry(entry)){
				...
				const parcelRequire = `parcelRequire(${JSON.stringify(this.bundleGraph.getAssetPublicId(entry))});\n`
				const entryExports = (_entry$symbols$get = entry.symbols.get("*")) === null || _entry$symbols$get === void 0 ? void 0 : _entry$symbols$get.local
		...
		const [postlude, postludeLines] = this.outputFormat.buildBundlePostlude()
		res += postlude
		...
		if(this.bundle.env.outputFormat === "global" && this.bundle.env.sourceType === "script"){
			res += "\n"
			lineCount++
			const mainEntry = (0, _nullthrows().default)(this.bundle.getMainEntry())
			const {
				code,
				map: mapBuffer
	...
	async loadAssets() {
		const queue = new(_utils().PromiseQueue)({
			maxConcurrent: 32
		})
		const wrapped = []
		this.bundle.traverseAssets(asset => {
			queue.add(async() => {
				const [code, map] = await Promise.all([asset.getCode(), this.bundle.env.sourceMap ? asset.getMapBuffer() : null])
				return [asset.id, {
					code,
					map
				}]
		...
		/** <PATCH> **/
		this.patch.prepareForAssetBuilding(this.assetOutputs)
		/** </PATCH> **/
	...
	buildExportedSymbols() {
		...
		const entry = this.bundle.getMainEntry()
	...
	buildAsset(asset, code, map) {
		...
		const deps = this.patch.getAssetDependencies(asset)
		/** </PATCH> **/

		const sourceMap = this.bundle.env.sourceMap && map ? new(_sourceMap2().default)(this.options.projectRoot, map) : null // If this asset is skipped, just add dependencies and not the asset's content.

		/** <PATCH> **/
		code = this.patch.updateAssetCode(asset, code, sourceMap)
		/** </PATCH> **/
		...
		if(depMap.size === 0 && replacements.size === 0){
			...
			code = code.replace(REPLACEMENT_RE, (m, d, i) => {
				...
				if(d != null){
					...
					/** <PATCH> **/
					if(!this.patch.shouldVisitAssetDependencyImports(asset)){
						return replacement
					}
					/** </PATCH> **/
				...
				const replacement = (_replacements$get = replacements.get(m)) !== null && _replacements$get !== void 0 ? _replacements$get : m
...
`
	...
	buildReplacements(asset, deps) {
		...
		const depMap = new(_utils().DefaultMap)(() => [])
		...
		for(const dep of deps){
			...
			/** <PATCH> **/
			const specialDependencyReplacement = this.patch.formSpecialDependencyReplacement(dep)
			if(specialDependencyReplacement){
				depMap.get(specialDependencyReplacement).push(dep)
			}
			/** </PATCH> **/
			const asyncResolution = this.bundleGraph.resolveAsyncDependency(dep, this.bundle)
			const resolved = (asyncResolution === null || asyncResolution === void 0 ? void 0 : asyncResolution.type) === "asset" // Prefer the underlying asset over a runtime to load it. It will
			...
			/** </PATCH> **/
				...
				/** <PATCH> **/
				// same as above, just with patched local name
				replacements.set(this.patch.updateSymbolLocalNameForReplacement(local, asset),
					(asyncResolution === null || asyncResolution === void 0 ? void 0 : asyncResolution.type) === "asset" ? `Promise.resolve(${symbol})` : symbol)
				/** </PATCH> **/
		...
		if(this.wrappedAssets.has(asset.id) || this.bundle.env.outputFormat === "commonjs" && asset === this.bundle.getMainEntry()){
			...
			const exportsName = ((_asset$symbols$get = asset.symbols.get("*")) === null || _asset$symbols$get === void 0 ? void 0 : _asset$symbols$get.local) || `$${assetId}$exports`
	...
	getSymbolResolution(parentAsset, resolved, imported, dep) {
		...
		const isWrapped = !this.bundle.hasAsset(resolvedAsset) || this.wrappedAssets.has(resolvedAsset.id) && resolvedAsset !== parentAsset
		const staticExports = resolvedAsset.meta.staticExports !== false
		const publicId = this.bundleGraph.getAssetPublicId(resolvedAsset) // If the resolved asset is wrapped, but imported at the top-level by this asset,
		...
		const obj = isWrapped && (!dep || dep !== null && dep !== void 0 && dep.meta.shouldWrap) // Wrap in extra parenthesis to not change semantics, e.g.`new (parcelRequire("..."))()`.
		...
		if(imported === "*" || exportSymbol === "*" || isDefaultInterop){
			...
			const kind = dep === null || dep === void 0 ? void 0 : dep.meta.kind
	...
	getHoistedParcelRequires(parentAsset, dep, resolved) {
		...
		let res = ""
		let lineCount = 0
		const isWrapped = !this.bundle.hasAsset(resolved) || this.wrappedAssets.has(resolved.id) && resolved !== parentAsset // If the resolved asset is wrapped and is imported in the top-level by this asset,
	...
	buildAssetPrelude(asset, deps) {
		...
		let append = ""
		const shouldWrap = this.wrappedAssets.has(asset.id)
		const usedSymbols = (0, _nullthrows().default)(this.bundleGraph.getUsedSymbols(asset))
		const assetId = asset.meta.id;
		...
		const defaultInterop = asset.symbols.hasExportSymbol("*") && usedSymbols.has("default") && !asset.symbols.hasExportSymbol("__esModule")
		const usedNamespace // If the asset has * in its used symbols, we might need the exports namespace.
    ...
    // this asset.
		...
		if(asset.meta.staticExports === false || shouldWrap || usedNamespace || defaultInterop){
				...
				for(const [imported, {
					...
					if(imported === "*" && local === "*"){
						if(!resolved){
							...
							const external = (0, _nullthrows().default)((0, _nullthrows().default)(this.externals.get(dep.specifier)).get("*"))
            ...
            || !resolved.meta.hasCJSExports && resolved.symbols.hasExportSymbol("*")){
							...
							for(const symbol of (0, _nullthrows().default)(this.bundleGraph.getUsedSymbols(dep))){
                ...
                || symbol === "__esModule"){
								...
								const set = asset.meta.hasCJSExports ? ", " + this.buildFunctionExpression(["v"], `${resolvedSymbol} = v`) : ""
			...
			const usedExports = [...asset.symbols.exportSymbols()].filter(symbol => {
				...
				const unused = incomingDeps.every(d => {
					const symbols = (0, _nullthrows().default)(this.bundleGraph.getUsedSymbols(d))
			...
			if(usedExports.length > 0){
				...
				prepend += `\n${usedExports.map(exp => {
					...
					const set = asset.meta.hasCJSExports ? ", " + this.buildFunctionExpression(["v"], `${resolved} = v`) : ""
	...
	buildBundlePrelude() {
		...
		if(mainEntry && !this.isAsyncBundle && !this.bundle.target.env.isBrowser()){
			const interpreter = mainEntry.meta.interpreter;
		...
		const [outputFormatPrelude, outputFormatLines] = this.outputFormat.buildBundlePrelude()
		...
		if(this.needsPrelude){
			...
			const mightBeFirstJS = parentBundles.length === 0 || parentBundles.some(b => b.type !== "js") || this.bundleGraph.getBundleGroupsContainingBundle(this.bundle).some(g => this.bundleGraph.isEntryBundleGroup(g)) || this.bundle.env.isIsolated() || this.bundle.bundleBehavior === "isolated"

			if(mightBeFirstJS){
				const preludeCode = (0, _helpers.prelude)(this.parcelRequireName)
				res += preludeCode
