...
/** <PATCH> **/
// dirname is acting weird
// and we need to also load prelude patch
const PRELUDE = _fs().default.readFileSync(_path().default.join(__dirname, "../parcel-packager-js-with-circular-dep-resolving/dev-prelude-patch.js"), "utf8").trim().replace(/;$/, "") + ";\n" + _fs().default.readFileSync(_path().default.join(__dirname, "../parcel-packager-js-with-circular-dep-resolving/dev-prelude.js"), "utf8").trim().replace(/;$/, "")
/** </PATCH> **/
...
class DevPackager {
	...
	async package() {
		...
		this.bundle.traverseAssets(asset => {
			queue.add(async() => {
				const [code, mapBuffer] = await Promise.all([asset.getCode(), this.bundle.env.sourceMap && asset.getMapBuffer()])
				return {
					code,
					mapBuffer
				}
		...
		let i = 0
		let first = true
		const map = new(_sourceMap().default)(this.options.projectRoot)
		...
		if(this.bundle.env.sourceType === "script" && script){
			let entryMap
			const mapBuffer = script.mapBuffer
	...
	getPrefix() {
		...
		if(mainEntry && this.isEntry() && !this.bundle.target.env.isBrowser()){
			const _interpreter = mainEntry.meta.interpreter;
